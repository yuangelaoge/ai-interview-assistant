...
const CONTEXT_EXTENSIONS = new Set(['.md', '.txt', '.pdf']);
...
export async function loadShallowContext(targetPath: string, maxChars = 10000): Promise<string> {
  return loadDocumentContext({
    ...
    directoryMessage: '浅层资料只支持单个 md、txt 或 pdf 文件，不支持文件夹。请在设置里选择一个整理好的上下文文件。',
    unsupportedMessage: '浅层资料只支持 md、txt 或 pdf 文件。请在设置里选择一个 .md、.txt 或 .pdf 文件。',
...
export async function loadDeepContext(targetPath: string, maxChars = 256000): Promise<string> {
  return loadDocumentContext({
    ...
    directoryMessage: '深答上下文只支持单个 md、txt 或 pdf 文件，不支持文件夹。请在设置里选择一个整理好的长上下文文件。',
    unsupportedMessage: '深答上下文只支持 md、txt 或 pdf 文件。请在设置里选择一个 .md、.txt 或 .pdf 文件。',
...
async function loadDocumentContext({
  ...
  const content =
    ext === '.pdf'
      ? await extractPdfText(targetPath)
      : await fs.promises.readFile(targetPath, 'utf8').catch(() => '');
