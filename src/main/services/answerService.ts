...
import { chatCompletion, streamChatCompletion, transcribeWithOpenAiCompatibleApi } from './openAiCompatible';
...
export async function generateFastAnswer(question: string): Promise<string> {
  const { config, messages } = await buildFastMessages(question);
  const content = await chatCompletion(config.fastModel, messages, {
    temperature: 0.25,
    maxTokens: 260,
...
export async function generateFastAnswerStream(question: string, onDelta: (d: string) => void): Promise<string> {
  const { config, messages } = await buildFastMessages(question);
  const content = await streamChatCompletion(config.fastModel, messages, onDelta, {
    temperature: 0.25,
    maxTokens: 260,
    thinking: config.fastModel.baseURL.includes('api.deepseek.com') ? 'disabled' : undefined
  });
...
async function buildFastMessages(question: string): Promise<{
