...
- 读取目录下 `.md` / `.txt` / `.pdf` 文件，分块后用 OpenAI 兼容 embeddings 向量化。
- 面试官问题触发快答或深答时，在内存中按余弦相似度检索 top-K 片段，并拼接到现有上下文前面。
- 单文件快答/深答上下文支持 `.pdf` 简历等资料，PDF 自动提取文本后沿用现有截断和 prompt 包装。
...
- 简历通常以 PDF 交付，当前 `.pdf` 会被文件过滤跳过，无法作为知识库片段或单文件上下文。
...
- PDF 文本提取通过 `pdf-parse` 完成，但必须用 `eval('require')` 间接动态加载；未安装或解析失败时记录 warn 并返回空文本，不让 App 崩溃。
- 复用现有 `openai` SDK 调 embeddings，Node `fs` 读目录，项目内实现余弦相似度。
...
- 不新增 OCR 能力；扫描版 PDF 如果提取不到文本，会按空文本降级。
...
- `pdf-parse` 加入运行依赖，打包配置可把 `node_modules/pdf-parse/**` 放入 `asarUnpack` 作为保险。
...
- 选择 `.pdf` 作为快答或深答上下文时，提取出的文本继续走现有 `trimContext` 截断和 `# 文件名` 包装。
- `pdf-parse` 未安装或 PDF 解析失败时，PDF 文本为空，其它 `.md` / `.txt` 文件读取和检索逻辑不受影响。
