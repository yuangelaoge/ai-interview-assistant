...
def run_preflight_external(workspace: Path, *, json_out: Path | None = None) -> int:
    figures_dir = output_dir(workspace)
    figures_dir.mkdir(parents=True, exist_ok=True)
    payload = {
        "ok": True,
        "mode": "external-render",
        "workspace": str(workspace),
        "outputDir": str(figures_dir),
        "checkedAt": utc_now(),
        "renderer": "manual external PNG supplied by user",
        "nextSteps": [
            "Codex writes figures/ai_generated/prompt_vN.md.",
            "User renders externally and provides figures/ai_generated/external_render_vN.png.",
            "Codex reviews, refines if needed, then runs finalize and verify.",
        ],
    }
    return emit_json(payload, json_out=json_out)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    preflight_external = subparsers.add_parser(
        "preflight-external",
        help="Check external-render workflow without requiring the Codex image bridge",
    )
    preflight_external.add_argument("--workspace", help="Paper or project workspace root")
    preflight_external.add_argument("--json-out", help="Optional path to save the JSON result")
...
def main() -> int:
    ...
    if args.command == "preflight-external":
        return run_preflight_external(workspace, json_out=json_out)
